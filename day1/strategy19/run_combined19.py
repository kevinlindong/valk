"""Run Strategy v19 + Probe over a single WS connection.

v19 = v18's dual-symbol MM + precompute hot path, but with fair_a/fair_b
sourced from the admin `/api/admin/truth` endpoint (the full reveal
sequence is known in advance). Snipe edges tighten to 0.5t, the
pre-reveal park / flatten-bias / no_info widen all drop out, and the
slow precompute collapses to a single truth-anchored scenario. See
`strategy19.py` for the design notes.

Run:
    python day1/strategy19/run_combined19.py
    python day1/strategy19/run_combined19.py --log /tmp/v19.jsonl
    python day1/strategy19/run_combined19.py --probe-symbol B

Interactive commands:
    s    status (pos A/B, fair A/B, truth seq, ramp, precompute size)
    t    truth status (seq, truth_a, truth_b@current_window)
...
from strategy19.strategy19 import (  # noqa: E402
    URL, API_KEY, N_PRIOR_SIM, Posterior, Strategy, Config,
)
from strategy19.probe19 import make_probe19, default_log_path  # noqa: E402
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Run Strategy v19 + Probe together over one WS connection")
    ...
    parser.add_argument("--log", default=None,
                        help="JSONL log path. Default: "
                             "day1/logs/combined_log_v19_<YYYYMMDD_HHMMSS>.jsonl")
    ...
    probe = make_probe19(c, symbol=args.probe_symbol, log_path=args.log)
    ...
    if c.game_state().get("phase") == "running":
        try:
            ...
            print(f"[v19 INIT-MM] {e!r}")

    print(f"\nCombined v19 runner started.")
    print(f"  Strategy v19: v18 execution + v16 truth oracle (admin)")
    print(f"  Probe:        JSONL log -> {args.log}  "
          f"(probe_symbol={args.probe_symbol})")
    print("Commands: 's'=status  't'=truth  'j'=trajectory  "
          "'p'=precompute  'c'=probe cp  'b'=books  'r'=rebuild  "
          "'f'=flatten  'q'=quit.\n")
    ...
    try:
        while True:
            ...
            if cmd == "s":
                ...
                print(f"  unknown command {cmd!r}; try s/t/j/p/c/b/r/f/q")
