"""
Run Strategy v6 (live trading) and Probe (passive market-data logger) together
over a SINGLE WebSocket connection.

The exchange allows only one private WS per API key, so you can't just run
strategy6.py and probe.py side-by-side -- one would steal the other's slot.
This runner shares one client and dispatches each WS event to both.

  - Strategy v6: layers PRE-REVEAL MM and an O(1) trade-tape rewrite on
                 top of v5. v5's K=0 sit-out left ~$300 of pre-reveal
                 edge on the table per round (combined_log_v5_
                 20260519_184605.jsonl shows book mid ~36.5 vs trade
                 VWAP ~20.8 vs settle 20 at K=0 -- bots quote wide and
                 a VWAP-anchored maker prints fills against them). v6
                 re-enables K=0 quoting ONLY when the trade VWAP is
                 fresh and backed by enough volume; pre-reveal fair is
                 a 70/30 blend of (VWAP, prior_mean), pre-reveal position
                 cap +/-5 lots. Snipes remain gated off until K>=1.
                 Also: _recent_trades is now a collections.deque with
                 running VWAP/imbalance sums so the hot path is O(1)
                 instead of O(n). BRYA added to informed_counterparties.
                 See strategy6.py docstring for the full rationale.
...
Run:
    ...
    python day1/run_combined6.py --log day1/sess.jsonl
    python day1/run_combined6.py --symbol A
    python day1/run_combined6.py --informed-trader ORACLE_NAME

The --informed-trader flag overrides strategy6.INFORMED_TRADER_ID. Pass
...
from strategy6 import URL, API_KEY, Posterior, Strategy, N_PRIOR_SIM  # noqa: E402
from probe import Probe  # noqa: E402


def main() -> None:
    parser = argparse.ArgumentParser(
        description="Run Strategy v6 + Probe together over one WS connection"
    )
    parser.add_argument("--symbol", default="A")
    parser.add_argument("--log", default=None,
                        help="JSONL log path. Default: "
                             "day1/logs/combined_log_v6_<YYYYMMDD_HHMMSS>.jsonl. "
    ...
    parser.add_argument("--informed-trader", default=None,
                        ...
                        help="Counterparty id of the informed trader who "
                             ...
                             "strategy6.INFORMED_TRADER_ID. Pass an empty "
                             "string to disable.")
    args = parser.parse_args()

    if args.log is None:
        ts = time.strftime("%Y%m%d_%H%M%S", time.localtime())
        args.log = f"day1/logs/combined_log_v6_{ts}.jsonl"
    ...
    try:
        while True:
            ...
            if cmd == "s":
                ...
                # v6: surface strike counters, cadence, informed truth
                # bounds, blended market signal + tape stats, and the
                # pre-reveal MM gate state when K=0.
                ...
                print(f"  phase={strat.phase}  pos={strat.position}  "
                      ...
                      f"{marker}\n"
                      f"  v6: maker_reprice_every={interval_str}  "
                ...
                # v6: market-tape state. Show VWAP/imbalance across the
