"""
Run Strategy v2 (live trading) and Probe (passive market-data logger) together
over a SINGLE WebSocket connection.

The exchange allows only one private WS per API key, so you can't just run
strategy2.py and probe.py side-by-side -- one would steal the other's slot.
This runner shares one client and dispatches each WS event to both.

  - Strategy v2: skips K=0 entirely (no pre-reveal quoting or sniping),
                 throttles maker reprices to a confidence-gated cadence
                 (4-12s depending on K), and widens per-side quotes based
                 on rolling "informed counterparty" strikes. Snipes remain
                 event-driven on quote_add. See strategy2.py docstring.
...
Run:
    python day1/run_combined2.py
    python day1/run_combined2.py --log day1/sess.jsonl
    python day1/run_combined2.py --symbol A
...
from strategy2 import URL, API_KEY, Posterior, Strategy, N_PRIOR_SIM  # noqa: E402
from probe import Probe  # noqa: E402


def main() -> None:
    parser = argparse.ArgumentParser(
        description="Run Strategy v2 + Probe together over one WS connection"
    )
    parser.add_argument("--symbol", default="A")
    parser.add_argument("--log", default=None,
                        help="JSONL log path. Default: "
                             "day1/logs/combined_log_v2_<YYYYMMDD_HHMMSS>.jsonl. "
    ...
    if args.log is None:
        ...
        args.log = f"day1/logs/combined_log_v2_{ts}.jsonl"
    ...
    try:
        while True:
            ...
            if cmd == "s":
                ...
                # v2: also surface strike counters and current cadence.
                ...
                print(f"  phase={strat.phase}  pos={strat.position}  "
                      ...
                      f"{marker}\n"
                      f"  v2: maker_reprice_every={interval_str}  "
