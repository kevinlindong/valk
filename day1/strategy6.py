"""
Day 1 live trading strategy v6.

Layers two changes onto v5: pre-reveal market making anchored to the
public trade VWAP, and a O(1) running-sum reimplementation of the
trade-tape statistics that v5 introduced.

POST-v5 LOG (combined_log_v5_20260519_184605.jsonl, 1 round, settle=20,
net PnL +291) revealed:

  1. PRE-REVEAL EDGE WAS UNTOUCHED. At K=0 the public book midpoint was
     36.5 while the 10s trade VWAP was already 20.8 (truth = 20). Other
     bots were taking 36-103 trades / round before the first reveal.
     We made ZERO fills at K=0 because `skip_pre_reveal=True` (set in
     v2 as a blanket defense after a tail-prior disaster). The right
     fix is conditional: enable K=0 quoting ONLY when a fresh trade
     VWAP exists with enough volume backing it, anchor the K=0 fair
     to a 70/30 blend of (VWAP, prior_mean), and cap pre-reveal
     position at +/-5 lots. The VWAP is fresh enough to defend against
     tail rounds (it incorporates real trades) while the small
     position cap bounds blow-up risk.

  2. PER-CP PnL AUDIT. BRYA's 4 fills against us netted -10 (similar
     to AARU's -10/3-fill profile). Add to `informed_counterparties`.

  3. TAPE STATS WERE O(n) ON A 500-ENTRY BUFFER. v5 used a plain list
     with O(n) trim and full scans in `_trade_vwap` /
     `_aggressor_imbalance`. `_market_signal` called both per event,
     and `_try_direct_snipe` fires on every quote_add (~17/s). The
     redundant scans matter because the snipe-race window is sub-50ms
     -- consistency, not just average, of latency is the win.

CHANGES vs v5:

  - `_recent_trades` is now `collections.deque(maxlen)`. `on_trade`
    pops expired entries from the head while subtracting them from
    running sums (`_vwap_num`, `_vwap_den`, `_imb_buy_q`,
    `_imb_sell_q`) and appends new entries with O(1) add. Hot-path
    `_trade_vwap()` / `_aggressor_imbalance()` are now O(1) for the
    buffer's native window (trade_window_sec=30s). The slow O(n)
    path is retained for arbitrary windows used by the 'm' status
    command.
  - Pre-reveal `desired_quotes` path: when K=0 AND _trade_vwap(30s)
    exists AND volume backing it >= `min_trade_vol_pre_reveal`,
    pre_reveal_fair = pre_reveal_vwap_weight * VWAP +
    (1 - w) * prior_mean. Quotes are posted at pre_reveal_fair
    +/- pre_reveal_min_edge (4 ticks), qty=1, position cap
    `pre_reveal_max_pos`=5. Skip K=0 quoting otherwise (current v5
    behavior -- no anchor, no trades).
  - `informed_counterparties` += {BRYA}.

CARRIED FORWARD from v5:
  - on_trade ring buffer, _market_signal book-mid + VWAP blend
    (now O(1) under the hood).
  - Per-CP signed flow tracking, cap-proximity strike decay
    (cp_cap_threshold=70, cp_near_cap_strike_decay=0.5).
  - Informed-trader override (`--informed-trader`), truth-bound clamp.
  - K=1+ logic from v4 unchanged: confidence-gated maker reprice
    cadence, strike-decay edge model, FIFO-priority modify guard,
    sigma-scaled position cap, symmetric snipe disagreement gate.

Run:
    python day1/run_combined6.py
...
class Strategy:
    def __init__(self, client: GameClient, posterior: Posterior, symbol: str = "A",
        ...
        self.min_trade_vol_pre_reveal: int = 8
