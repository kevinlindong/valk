"""
Day 1 live trading strategy v2.

Same Bayesian-posterior core as strategy.py, with three log-driven changes:

  1. SIT OUT K=0 ENTIRELY. Pre-reveal we have no information beyond the
     prior; quoting then is purely adverse-selection exposure. Log analysis
     across 11 sessions: 10 had ZERO pre-reveal fills for us, and the one
     that did was a +0.3/lot wash. Prior-only sigma ~13 so any quote tighter
     than 13 leaks edge. v2 does nothing until reveal #1 lands: no quotes,
     no book fetches, no snipe checks, no penny logic.

  2. CONFIDENCE-GATED MAKER REPRICE CADENCE. Log analysis showed our
     same-order modify_acks fired with median 60-160 ms gaps -- ~10x/sec
     against a fair that only moves materially every 30 s (each reveal).
     Most modifies chased top-of-book noise and forfeited queue priority
     for nothing. v2 throttles MAKER repricing per-phase:
        K=1-3:  every 12 s   (sigma ~12, fair moves slowly between reveals)
        K=4-7:  every 8 s    (sigma ~8)
        K=8-10: every 4 s    (sigma ~4 down to 0; last reveals matter most)
     Reveal arrival forces an immediate reprice (timer reset). SNIPES
     remain event-driven on every quote_add -- they are the signal.

  3. COUNTERPARTY-AWARE QUOTE WIDTH. Log analysis identified a stable set
     of "informed" bots (VALKC/VALKM/VALKN/VALKJ) whose fills against us
     net -1 to -3 per lot at settlement, vs safe-flow bots (VALKG/VALKB/
     VALKK/CHAR/...) where we win +500 to +1000 per session. v2 keeps
     per-side "informed strike" counters that decay exponentially (~60 s
     half-life). Each fill against an informed bot adds +1 strike on the
     side we got picked off; strikes add 0.5 ticks of extra edge per
     strike on that side, capped, until they decay. Adaptive width
     without permanently widening against the safe flow we want to capture.

Other behaviors carry forward unchanged from strategy.py:
  - Posterior, fair_and_sigma cache, FIFO-priority-preserving _reprice guard
    (rest_qty <= target -> skip), per-order modify debounce, SDK token-bucket
    rate-limit guard, _try_direct_snipe path, market-mid disagreement gate
    for snipes, sigma-scaled position cap, inventory-aware snipe edge,
    post-final-reveal sweep, on-reveal cancel-all defense.

Run:
    python day1/strategy2.py
