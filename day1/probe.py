"""
Market microstructure probe.

Connects to the exchange and characterizes the trading behavior of the OTHER
participants -- this market has multiple humans and multiple bots, so we both
log everyone we trade with (per-counterparty stats) AND keep probes small and
infrequent so we don't grief the humans.

Three layers:

  1. Passive logger: every reveal, fill, trade on the public tape, book delta,
     ack, reject, and raw message gets appended to a JSONL log. Always on.

  2. Counterparty tracker: every fill (us) and trade (tape) updates per-
     counterparty stats -- volume on each side, maker/taker split, average
     price. Press 'c' to print the table. This is where you spot patterns
     like "counterparty X always sells into my bid 200ms after a high reveal".

  3. Active probes: optional small (qty=1) probe orders sent on a cadence,
     each followed by a series of book snapshots to measure how others react.
     Default interval is 30s -- intentionally polite so we don't antagonize
     human players. Sub-modes:

       repost      Cross the spread by 1 lot at best ask, then sample the book
                   at 50ms, 100, 200, 500, 1000, 2000, 5000 ms after. Reveals
                   per-MM repost behavior (speed, price drift, inventory aversion).
       fronting    Post a passive bid 1 tick INSIDE the best bid (becomes new
                   best). Sample book over 3s. Tells you who joins, fronts, or
                   ignores a new top-of-book quote.
       snipe_lat   Post a passive ask 1 tick inside the best ask (becomes new
                   best). Time until lifted (or 5s timeout). Lower-bound on
                   any sniper's edge requirement and latency.
       all         Rotate through all three with a 2s gap between each.

  Run during the 'running' phase only -- probes outside that are skipped.

Safety + politeness:
  - PROBE_SIZE = 1
  - MAX_PROBE_POS = 5  (auto-flatten if |position| exceeds this)
  - Probes are skipped if a flatten would breach the position limit anyway
  - Default probe interval is 30s (don't spam other humans)
  - --exclude-counterparty NAME  skips a probe if our most recent fill was
    against NAME (useful if you trade with a specific human and want to
    avoid pestering them in particular)
  - 'f' command flattens immediately; 'q' (or Ctrl-C) flattens then exits

Run:
    python day1/probe.py                                  # passive + tracker
    python day1/probe.py --mode repost
    python day1/probe.py --mode all --probe-interval 60
    python day1/probe.py --mode passive --log day1/probe_today.jsonl

Logging:
    By default each run writes to day1/logs/probe_log_<YYYYMMDD_HHMMSS>.jsonl,
    so sessions never overwrite or interleave with each other. If you pass --log
    explicitly, that file is opened in APPEND mode (existing data preserved).
    The logs/ directory is created automatically.

Interactive:
    s     status (phase, position, top of book)
    c     print per-counterparty fill stats
    f     flatten now
    q     quit (auto-flattens)

Analyze (rough):
    jq -c 'select(.kind=="fill")'   day1/probe_log.jsonl
    jq -c 'select(.kind=="trade")'  day1/probe_log.jsonl  | head
    jq -c 'select(.kind=="probe_book_after")' day1/probe_log.jsonl
    jq -c 'select(.kind=="counterparty_summary")' day1/probe_log.jsonl  | tail -1
"""
...
# Allow `python day1/probe.py` from project root.
...
MAX_PROBE_POS = 5       # auto-flatten if |position| exceeds this
BOOK_THROTTLE_SEC = 1.0 # max one periodic book log per second
...
class Probe:
    def __init__(
        ...
        self._last_periodic_book_t = 0.0
    ...
    def on_book(self, msg: dict) -> None:
        self._touch()
        now = time.time()
        if now - self._last_periodic_book_t > BOOK_THROTTLE_SEC:
            self._last_periodic_book_t = now
            self.log("book_periodic", **msg)
