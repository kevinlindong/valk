"""
Day 1 live trading strategy v7.

Adds a "don't fight the tape" layer on top of v6. Built from the
sean_bot1..9 log audit (2026-05-19), where a participant identified as
SEAN extracted ~$150 across ~5 fills by posting offers at VWAP + 2-5
ticks and waiting for us to walk through them. The pattern in bot6:
SEAN sold 23 lots @ 21-22 to us while the 30s trade VWAP was 17-20 and
the round settled at 16. Our posterior-extrapolated fair at K=2 was 25
(rs/K * 10 = 5/2 * 10), so we lifted offers that the broader market
was selling INTO -- and SEAN was the cleanest expression of that
"market consensus" anchor.

The lesson is general: at low K the posterior is dominated by the
prior and a tiny sample, while the trade tape integrates information
across ALL participants. When the two disagree we should trust the
tape more at K=0-2 and gradually shift to the posterior as K grows.
The existing `snipe_max_disagreement` gate (set to 5.0 in v3) catches
some of these but is too lax at low K -- bot6 K=2 had |diff|=4.4,
just under the threshold.

CHANGES vs v6 (each is a knob; defaults conservative; nothing is
bot-specific):

  1. VWAP-CLAMPED FAIR AT LOW K. New helper `_vwap_clamped_fair(fair)`.
     When 1 <= K <= `vwap_clamp_max_k` AND the trade tape has >=
     `vwap_clamp_min_volume` lots in the last `trade_window_sec`, pull
     `fair_eff` toward the VWAP by weight
       w = (vwap_clamp_max_k - K + 1) / (vwap_clamp_max_k + 1)
     so for the default `vwap_clamp_max_k = 3`:
       K=1: w = 3/4 = 0.75
       K=2: w = 2/4 = 0.50
       K=3: w = 1/4 = 0.25
       K>=4: w = 0 (no clamp; posterior is informative enough)
     K=0 is handled separately by `_pre_reveal_v6_fair` (already a
     VWAP+prior blend), so the helper bails out there to avoid
     double-clamping. This affects BOTH maker quoting (desired_quotes)
     and snipe edge math (maybe_snipe / _try_direct_snipe).

  2. VWAP SIDE GUARD ON MAKER QUOTES. After computing bid_px / ask_px
     in desired_quotes, when VWAP is fresh enough (>= vwap_clamp_min_volume
     lots in `trade_window_sec`):
       bid_px <= floor(vwap) - vwap_side_guard_ticks
       ask_px >= ceil(vwap)  + vwap_side_guard_ticks
     Never tightens (only widens away from VWAP). This prevents the
     "post a bid above the trade VWAP" failure mode -- a bid above
     VWAP almost always means our posterior disagrees with consensus
     and the consensus is right.

  3. VWAP DIRECTION-AGREEMENT GATE ON SNIPES. Before firing a snipe
     in maybe_snipe / _try_direct_snipe, require that the VWAP agrees
     with the snipe direction (within `snipe_vwap_buffer` ticks):
       BUY  snipe at level price P needs vwap >= P - snipe_vwap_buffer
                 (VWAP doesn't say the market is trading below P;
                  i.e., P is not "overpriced relative to consensus")
       SELL snipe at level price P needs vwap <= P + snipe_vwap_buffer
                 (VWAP doesn't say the market is above P)
     This is "don't fight the tape": when our fair-vs-level edge says
     buy but VWAP says the level is above market, we abstain. In
     bot6, every SEAN snipe we ate would have been blocked here
     (VWAP 17-20 vs level prices 21-22).

  4. POST-PARTIAL-FILL COOL-OFF (maker). When one of our resting
     quotes gets a maker fill and qty remaining > 0, mark the side
     with a `partial_fill_cool_off_sec` cool-off. desired_quotes
     suppresses re-posts on that side during the cool-off so we
     don't immediately re-quote into a moving market. This is
     symmetric to the existing informed-trader cool-off but fires
     on any maker partial -- maker partials are the cleanest signal
     that the other side has new information.

CARRIED FORWARD from v6:
  - O(1) running-sum trade tape, K=0 VWAP-anchored MM
    (`_pre_reveal_v6_fair`), informed-trader override, per-CP signed
    flow tracking, cap-proximity strike decay, confidence-gated maker
    reprice cadence, FIFO-priority modify guard, sigma-scaled position
    cap, symmetric snipe `snipe_max_disagreement` gate (now a back-up
    to the new clamp + direction gate).

Run:
    python day1/run_combined7.py
While running: 's'=status, 'i'=informed-trader, 'm'=market-tape,
'f'=flatten, 'q'=quit.
"""
...
class Strategy:
    def __init__(self, client: GameClient, posterior: Posterior, symbol: str = "A",
        ...
        # v6: per-CP PnL audit of combined_log_v5_20260519_184605.jsonl:
        #   BRYA : -10 over 4 lots (~-2.5/lot, consistently picks us
        #          off mid-round) -- ADD to informed set. Similar
        #          profile to AARU.
        ...
        # v6 public trade tape. deque-backed ring buffer of
        # (t, price, qty, aggressor) for the last `trade_window_sec`.
        # Populated by Strategy.on_trade (wired from the SDK on_trade
        # callback in run_combined6). Used by _trade_vwap /
        # _aggressor_imbalance / _market_signal.
        #
        # v6 maintains running sums alongside the buffer so the
        # native-window VWAP and aggressor-imbalance lookups are O(1):
        #   _vwap_num  = sum(price * qty)  over the buffer
        #   _vwap_den  = sum(qty)          over the buffer
        #   _imb_buy_q = sum(qty when aggressor == "buy")
        #   _imb_sell_q = sum(qty when aggressor == "sell")
        # Expiring entries are popped from the head in on_trade and
        # subtracted from the sums. trade_window_sec is shortened to
        # 30 s (was 60) so the buffer length matches the window used
        # by _market_signal / _trade_vwap (no second scan needed).
        ...
        # v6 pre-reveal MM gate. Default v5 behavior is to skip K=0
        # entirely (`skip_pre_reveal=True`). v6 re-enables K=0 quoting
        # but ONLY when the public trade VWAP is fresh and backed by
        # at least `min_trade_vol_pre_reveal` lots of volume across
        # the buffer window. The K=0 quoting fair is a blend:
        #   pre_reveal_fair = w * trade_vwap + (1 - w) * prior_mean
        # with w = pre_reveal_vwap_weight. The position cap and per-
        # side qty are tight (pre_reveal_max_pos, pre_reveal_quote_qty)
        # because we still don't have a posterior to bound exposure.
        # If the VWAP is missing or thin we fall through to the v5
        # "skip K=0" behavior.
        # Log evidence (combined_log_v5_20260519_184605.jsonl): at K=0
        # the book midpoint was 36.5 but the 10s VWAP was already 20.8
        # and the round settled at 20. Other bots logged 36-103 K=0
        # trades while we made 0. With the VWAP anchor and a +/-5
        # position cap, the worst-case K=0 fill is ~5 * pre_reveal_min_edge
        # of misalignment vs truth -- a small bounded loss.
        self.pre_reveal_mm_enabled: bool = True
        # Lowered 8 -> 3 after sean_bot2 review. With the symbol filter
        # in on_trade, even a handful of A-only lots is a stronger
        # signal than the pre-K=0 prior (E[settle]~27 vs typical
        # market consensus 35-45 in the first second). The original
        # floor of 8 prevented the v6 blend from engaging in time --
        # at t+2.5s of sean_bot2, only 4 symbol-A lots had traded,
        # the blend bailed, and fair fell back to the (too-low) prior.
        self.min_trade_vol_pre_reveal: int = 3
        self.pre_reveal_vwap_weight: float = 0.7
        self.pre_reveal_max_pos: int = 5

        # v7 "don't fight the tape" knobs. See module docstring for the
        # full sean_bot1..9 audit and the bot6 smoking-gun trace.
        #
        # vwap_clamp_max_k: at K <= this value, pull fair toward the
        # public trade VWAP. At K=0 the v6 pre-reveal blend already
        # consumes the VWAP, so the clamp only kicks in once we have
        # the first reveal (K>=1) and continues decaying until K >
        # vwap_clamp_max_k. The blend weight at K is
        #   w = (vwap_clamp_max_k - K + 1) / (vwap_clamp_max_k + 1)
        # so for vwap_clamp_max_k = 3:
        #   K=0: helper bails (handled by _pre_reveal_v6_fair instead)
        #   K=1: w = 3/4 = 0.75
        #   K=2: w = 2/4 = 0.50
        #   K=3: w = 1/4 = 0.25
        #   K>=4: w=0 (no clamp; posterior is informative enough)
        # vwap_clamp_min_volume: gate on the VWAP. The buffer needs at
        # least this many lots in the last trade_window_sec before we
        # trust the VWAP enough to pull our posterior toward it. Below
        # the floor (or empty), the helper returns fair unchanged.
        self.vwap_clamp_max_k: int = 3
        # Lowered 8 -> 3 in lockstep with min_trade_vol_pre_reveal: the
        # clamp and side guard are the K>=1 counterparts of the v6 blend,
        # and the same "engages too late" argument applies.
        self.vwap_clamp_min_volume: int = 3

        # vwap_side_guard_ticks: minimum extra distance our maker
        # quotes keep from the trade VWAP. Specifically:
        #   bid_px <= floor(vwap) - vwap_side_guard_ticks
        #   ask_px >= ceil(vwap)  + vwap_side_guard_ticks
        # Never tightens (only widens away from VWAP). Set to 0 to
        # disable. Defends against the bot6 failure where we posted
        # bids near 22 while VWAP was 20 -- a maker bid ABOVE VWAP
        # is almost always a posterior/consensus disagreement and the
        # consensus is closer to truth.
        self.vwap_side_guard_ticks: int = 1

        # snipe_vwap_buffer: ticks of slack on the VWAP direction gate
        # applied inside maybe_snipe / _try_direct_snipe. Before a
        # BUY snipe at level price P, require vwap >= P - buffer
        # (the tape doesn't disagree with our intent to buy at P).
        # Before SELL, require vwap <= P + buffer. The gate fires only
        # when VWAP is fresh (>= vwap_clamp_min_volume in the window);
        # missing VWAP falls back to the snipe_max_disagreement gate.
        # Buffer = 1 is enough to let through "VWAP == our snipe level"
        # cases (we're at consensus, just one tick off).
        self.snipe_vwap_buffer: float = 1.0

        # partial_fill_cool_off_sec: when our resting maker gets a
        # PARTIAL fill (qty consumed < original resting qty), pause
        # re-posting on that side for this long. Maker partials are
        # the cleanest signal that the counterparty has new info and
        # is walking through us; re-posting immediately just feeds
        # the next leg of the walk. The cool-off is symmetric to the
        # informed-trader cool-off but fires on ANY maker partial
    ...
    def _pre_reveal_v6_fair(self, prior_mean: float) -> Optional[float]:
        """v6: K=0 maker anchor. Returns the VWAP-prior blended fair if
        all pre-reveal MM gates pass, else None. Called from
        desired_quotes when K=0. Gates:
          - pre_reveal_mm_enabled
          - trade VWAP exists (running sum non-empty)
          - VWAP volume >= min_trade_vol_pre_reveal lots
        `prior_mean` is the posterior-derived fair at K=0 (passed in by
        the caller, which already has it via fair_and_sigma()).
        """
    ...
    def _vwap_clamped_fair(self, fair: float) -> float:
        """v7: at low K, pull `fair` toward the public trade VWAP.

        Weighting (linear-decay):
            w = (vwap_clamp_max_k - K + 1) / (vwap_clamp_max_k + 1)
        so the clamp tapers off as we accumulate reveals and the
        posterior becomes informative on its own. For the default
        vwap_clamp_max_k=3: w = 0.75, 0.50, 0.25, 0 at K = 1, 2, 3, >=4.

        Gates (any failing -> return fair unchanged):
          - K <= vwap_clamp_max_k
          - VWAP exists and has >= vwap_clamp_min_volume lots in window
          - K >= 1 (K=0 path already handled by _pre_reveal_v6_fair,
            which blends VWAP with the prior; we don't double-clamp)

        Hot path: called from desired_quotes, maybe_snipe,
        _try_direct_snipe. Cheap -- one running-sum read plus a
        weighted average.
        """
        k = len(self.posterior.reveals)
    ...
    def _vwap_for_side_guard(self) -> Optional[float]:
        """v7: trade VWAP suitable for the maker-side guard. Returns
        the VWAP only if the buffer has >= vwap_clamp_min_volume lots
        in window; else None so callers fall through.
        """
        if self._vwap_den < self.vwap_clamp_min_volume:
            return None
    ...
    def _expire_trades(self, now: float) -> None:
        """Pop expired entries from the head of `_recent_trades` and
        subtract them from the running sums. Called from `on_trade` (so
        every append maintains tight bookkeeping) AND from the read-side
        helpers (so a long lull between trades doesn't leave stale
        weight in the running sums when the next reader queries them).
    ...
    def on_trade(self, msg: dict) -> None:
        """v6: public trade tape. Each entry is (t, price, qty,
        aggressor). `aggressor` is "buy" when the taker bought (lifted
        an ask) and "sell" when the taker sold (hit a bid).
        Expiry + running-sum bookkeeping are O(1) amortized: the deque
        pops from the head, sums are added on append and subtracted on
        evict.

        BUGFIX: symbol B has a totally different price regime (typical
        settle 1-8 vs symbol A ~30). Mixing both into one VWAP buffer
        poisoned every v6/v7 VWAP defense (fair clamp, side guard, snipe
        VWAP-direction gate). Cross-symbol contamination explained the
        K=0 maker-cross disaster in strat7_sean_bot2 (settle=53, -427.5
        PnL): the symbol-B trades dragged VWAP from the true 39.75 down
        to 22.6, the side guard's ask_floor became ~24 instead of ~41,
        and our maker SELL @ 38 crossed the bid @ 40 to fill as TAKER.
        Filter to the strategy's own symbol before the running sums
        update.
        """
        ...
        with self.lock:
            ...
            # The deque is bounded by maxlen. If we hit the cap, the
            # leftmost entry is evicted by append; subtract it from
            # the running sums BEFORE losing it.
    ...
    def _trade_vwap(self, window: float = 30.0) -> Optional[float]:
        ...
        Fast path (O(1)): when `window` >= trade_window_sec, we serve
        the running-sum directly after expiring stale entries.
        Slow path (O(n)): for shorter windows (used by the 'm' status
        command), we scan the buffer. Both paths return the same value
        in the common case where window == trade_window_sec.
        """
        if window <= 0:
            return None
        now = time.time()
        with self.lock:
            ...
            # Slow path for a sub-buffer window.
            cutoff = now - window
            num = 0.0
            den = 0
    ...
    def _aggressor_imbalance(self, window: float = 30.0) -> Optional[float]:
        ...
        Same fast/slow split as _trade_vwap.
        """
        if window <= 0:
            return None
        now = time.time()
    ...
    def _market_signal(self) -> Optional[float]:
        ...
        v6: VWAP path is O(1) (running-sum direct read for the buffer's
        native window). Book-mid path is unchanged.
        """
    ...
    def desired_quotes(self) -> tuple[Optional[int], Optional[int], float, float]:
        ...
        # v7: at low K, pull fair toward the trade VWAP so our
        # posterior-driven quote anchor doesn't fight the broader
        # market's consensus. K=0 is handled separately by
        # _pre_reveal_v6_fair below (so no double-clamp); K=1..3
        # apply a tapered blend (see _vwap_clamped_fair).
        fair_eff = self._vwap_clamped_fair(fair_eff)
        # v6: K=0 pre-reveal MM. Default v5 behavior is to skip K=0
        # entirely (skip_pre_reveal=True). v6 re-enables K=0 quoting
        # if and only if the public trade VWAP gate fires. When it
        # does, replace `fair_eff` with the VWAP+prior blend so all
        # downstream pricing math (edge, market-mid anchor, empty-side
        # widening) uses a market-aware anchor instead of the prior
        # alone. The raw `fair`/`sigma` returned to the caller stay
        # the posterior values (used for status display only).
        pre_reveal_v6_active = False
        if len(self.posterior.reveals) == 0:
            v6_fair = self._pre_reveal_v6_fair(prior_mean=fair)
            if v6_fair is None:
                # No qualifying VWAP -- preserve v5's skip behavior.
        ...
        # v7: VWAP side guard. After all the price math, hold each
        # maker quote at least vwap_side_guard_ticks away from the
        # public trade VWAP -- bid below floor(vwap), ask above
        # ceil(vwap). Only widens; never tightens. Defends against
        # the bot6 failure where we posted bids above VWAP because
        # our K=2 posterior was anchored too high.
        ...
        # v6: when the K=0 VWAP-anchored MM path is active, override the
        # cap with the tighter pre_reveal_max_pos. The posterior sigma at
        # K=0 is huge (~13) and the relaxed cap derived from it is still
        # too generous given we have no info beyond the prior. Cap at
        # +/-5 lots so a worst-case mis-anchored round bleeds at most
        # 5 * (pre_reveal_min_edge + sigma) ticks.
        ...
        # v7: post-partial-fill cool-off. Any maker partial -- not
        # just informed -- is a strong signal that the other side
        # has new info and is walking through us. Sit the side out
        # for partial_fill_cool_off_sec before reposting so we
        # don't keep feeding the walk.
        if (bid_px is not None and
        ...
        # v7.1: no-cross guard. The hardest invariant a maker quote
        # can violate is "post a limit that's already marketable" --
        # we get instantly taker-filled at the inside, paying the
        # taker fee AND giving up the maker rebate. This actually
        # happened in sean_bot2 K=0: fair was anchored to a (then
        # symbol-mixed, too-low) prior, ask_eff came out to 38, the
        # market bid was 40, our limit SELL @ 38 routed against the
        # bid and we filled three times as TAKER @ 40. The VWAP side
        # guard above is the soft defense (anchored to a trailing
        # window), this is the hard floor: if the cached inside book
        # says we'd cross, drop the post and wait for the next reprice.
        # Uses the same cache _apply_penny consults; staleness check
        # mirrors it so we don't act on freshly-empty book payloads.
        book = self._book_cache
        if (book is not None
    ...
    def maybe_snipe(self, fair: float, sigma: float) -> bool:
        ...
        fair_eff = self._effective_fair(fair)
        # v7: also pull fair toward VWAP at low K. Same rationale --
        # the broader market's tape integrates more info than a 1-2
        # sample posterior. Bot6 failure: our K=2 fair was 25 vs
        # VWAP 20, so we lifted offers at 21-22 and the round
        # settled at 16.
        fair_eff = self._vwap_clamped_fair(fair_eff)

        # v7: trade VWAP for the snipe direction gate. Fresh-only
        # (the `vwap_clamp_min_volume` floor); None falls back to
        # the existing snipe_max_disagreement gate below.
        ...
        if not block_buy:
            for level in book.get("asks") or []:
                ...
                if mispricing <= edge_required_buy:
                    break
                # v7: VWAP direction gate. Don't BUY at a level that
                # the trade tape already says is overpriced vs
                # consensus. Mid-round only -- post-final-reveal
                # sweep skips this because settle is known.
        ...
        if not block_sell:
            for level in book.get("bids") or []:
                ...
                if mispricing <= edge_required_sell:
                    break
                # v7: VWAP direction gate (symmetric). Don't SELL at
                # a level that the tape says is underpriced vs
                # consensus.
                if (snipe_vwap is not None and self._n_remaining() > 0
                        and snipe_vwap > level["price"] + self.snipe_vwap_buffer):
                    break
    ...
    def _try_direct_snipe(self, msg: dict) -> bool:
        ...
        with self.lock:
            ...
            fair_eff = self._effective_fair(fair)
            # v7: pull fair toward VWAP at low K (same as maybe_snipe).
            fair_eff = self._vwap_clamped_fair(fair_eff)
            # v7: fresh-VWAP for direction gate; None falls back.
            snipe_vwap = self._vwap_for_side_guard()
            ...
            if side == "buy":
                ...
                if mispricing <= edge_required_sell:
                    return False
                # v7: VWAP direction gate. Don't SELL into a level the
                # tape says is underpriced relative to consensus.
                if (snipe_vwap is not None and self._n_remaining() > 0
                ...
                if mispricing <= edge_required_buy:
                    return False
                # v7: VWAP direction gate. Don't BUY at a level the
                # tape says is overpriced relative to consensus.
                if (snipe_vwap is not None and self._n_remaining() > 0
                        and snipe_vwap < price - self.snipe_vwap_buffer):
                    return False
                headroom = snipe_max_pos - self.position
    ...
    def step(self, *, reconcile: bool = False) -> None:
        with self.lock:
            ...
            # v6: K=0 now runs through desired_quotes -- it short-circuits
            # internally when the VWAP gate fails. Snipes still respect
            # min_reveals_to_snipe inside maybe_snipe (it's a no-op at K=0).
            pre_reveal = (len(self.posterior.reveals) == 0)
            ...
            for key, expected_side in (("bid", "buy"), ("ask", "sell")):
                ...
                if rest is not None and rest["order_id"] == order_id and side == expected_side:
                    ...
                    if side == "buy":
                        ...
                        self.position -= qty
                    # v7: maker partial-fill cool-off. If our maker
                    # quote was only partly consumed, the taker is
                    # likely walking through us with new info -- pause
                    # re-posting on this side for
                    # `partial_fill_cool_off_sec`. Full fills don't
                    # trigger (no surviving qty -> no stale price to
                    # protect; the next post will be at a fresh fair).
                    if (rest["qty"] > 0
    ...
    def on_quote_event(self, msg: dict) -> None:
        ...
        Pre-reveal (K=0): snipes short-circuit (no posterior to bound
        edge). Maker reprice runs only if v6's VWAP-anchored gate
        accepts; otherwise desired_quotes returns (None, None) and the
        apply_target call is a no-op.
        """
        if self.phase != "running":
            return
        k = len(self.posterior.reveals)
        ...
        # Path 1: direct snipe on every quote_add. Snipes still respect
        # min_reveals_to_snipe inside _try_direct_snipe, so the K=0 case
        # is a no-op there.
        if not pre_reveal and msg.get("type") == "quote_add":
            self._try_direct_snipe(msg)

        now = time.time()
        # Path 2: throttled book scan for fallback snipes (skipped at K=0).
        do_book_scan = (not pre_reveal and
                        (now - self._last_quote_event_t >=
                         self.quote_event_throttle_sec))
        # Path 3: confidence-gated maker reprice. At K=0 the v6 path
        # provides its own cadence via desired_quotes -- use a fixed
        # 1s cadence to keep the pre-reveal quote drift-aligned with
        # incoming trade ticks (the trade tape updates every ~50-200ms;
        # 1s is enough to track without flooding the modify channel).
        if pre_reveal:
        ...
        with self.lock:
            ...
            if do_book_scan and k >= self.min_reveals_to_snipe:
                ...
                self.maybe_snipe(fair, sigma)
            if do_maker_reprice:
                # v6: at K=0, desired_quotes returns (None, None) when
                # the VWAP gate fails. Calling it cheaply is fine.
    ...
    def on_phase_change(self, phase: Optional[str], reveals: list[float]) -> None:
        with self.lock:
            ...
            if phase == "running":
                ...
                # v7: clear partial-fill cool-offs; per-round signal.
                ...
                # changed.
                # v6: also reset the running sums alongside the deque.
