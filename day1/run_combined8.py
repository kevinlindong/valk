"""Run Strategy v8 (live trading) and Probe (passive market-data logger) together
over a SINGLE WebSocket connection.

  - Strategy v8: SEAN-window passive MM layered on top of v7. Background
                 scheduler thread cancels quotes ~0.3s before each
                 predicted reveal and re-posts (upsized) ~1.5s before.
                 Adds a reveal-math fair guard that blocks aggressive
                 snipes when the posterior disagrees with the sample-
                 mean projection. Aggression knobs tightened.
                 See strategy8.py docstring for the full audit-grounded
                 rationale.
...
Run:
    ...
    python day1/run_combined8.py --log day1/sess.jsonl
    python day1/run_combined8.py --symbol A
    python day1/run_combined8.py --informed-trader ORACLE_NAME

The --informed-trader flag is passed through to the strategy (same
semantics as v7).

Interactive:
    s   strategy status (v8: SEAN-window + reveal-math fair surfaced)
...
from strategy8 import URL, API_KEY, Posterior, Strategy, N_PRIOR_SIM  # noqa: E402
from probe import Probe  # noqa: E402


def main() -> None:
    parser = argparse.ArgumentParser(
        description="Run Strategy v8 + Probe together over one WS connection"
    )
    parser.add_argument("--symbol", default="A")
    parser.add_argument("--log", default=None,
                        help="JSONL log path. Default: "
                             "day1/logs/combined_log_v8_<YYYYMMDD_HHMMSS>.jsonl. "
    ...
    if args.log is None:
        ...
        args.log = f"day1/logs/combined_log_v8_{ts}.jsonl"
    ...
    print(f"[v8] reveal_interval={strat.reveal_interval}s  "
          ...
          f"sean_pre_reveal_cancel={strat.sean_pre_reveal_cancel_sec}s  "
    ...
    print(f"  Strategy v8: live trading (symbol={args.symbol})")
        ...
        while True:
            ...
            if cmd == "s":
                ...
                print(f"  phase={strat.phase}  pos={strat.position}  "
                      ...
                      f"{marker}\n"
                      f"  v8: in_sean_window={in_window}  "
                      f"next_reveal_in={nxt_in_str}  "
                      f"sean_qty_target={strat.sean_window_qty}  "
