"""
Day 1 live trading strategy v3.

Same Bayesian-posterior core as strategy2.py. v3 hardens snipe-side risk
controls after a forensic review of second_round_loss.jsonl (-$816.59 in
a single round) and adds two structural guards around the feed delay
between public and private market data:

  1. SYMMETRIC DISAGREEMENT GATE. v2 ran with snipe_disagreement_directional
     = True, which allowed sells (or buys) "with our conviction" through
     even when |fair - market_mid| was large. In a tail-DOWN round our
     posterior collapsed tight on low observed values while market_mid
     stayed near consensus -- the directional override fired SELL snipes
     into bids that turned out to be correct, contributing ~$330 of the
     loss. v3 sets `snipe_disagreement_directional = False`: any snipe
     beyond `snipe_max_disagreement` is blocked, both directions.

  2. TIGHTER DISAGREEMENT THRESHOLD. v2's snipe_max_disagreement = 10.0
     was wider than the maker-side market-anchor threshold of 5.0 (we
     trusted the market for quote anchoring but not for snipe gating).
     v3 aligns them: `snipe_max_disagreement = 5.0`. Snipes only fire
     when our fair is within 5 ticks of consensus; otherwise we defer.

  3. POSITION-BUFFER ON SNIPES. The loss session went to |pos|=127 vs
     the ±100 hard limit -- the exchange clamped fills, leaving us with
     mismatched trade records and ~$190 of forced bad fills. v3 adds
     `snipe_position_buffer = 5`: every snipe (book-scan AND direct)
     must leave at least 5 lots of headroom before the hard ±100 limit.
     Maker quotes still use the full sigma-scaled cap; only snipes are
     buffered (snipe IOCs are the path that exceeded position limit).

  4. NO-COUNTERPARTY MAKER GATE. Market-making earns its spread by
     intermediating OTHER participants' flow; quoting into an empty
     book (or one that contains only our own resting orders) either
     decays into nothing or risks matching against ourselves on a
     subsequent reprice. v3 adds `_other_makers_present()` as a hard
     gate in desired_quotes -- if the book has no non-self levels, we
     don't post. Snipes remain reactive (they fire on specific
     quote_add events). Most relevant at the very start of a round
     before other bots have warmed up.

  5. PRIVATE-FEED-FIRST MARKET SIGNAL. The public book/quote feed
     lags the private fill/ack feed by ~50-200 ms. v3 tracks the
     most-recent observed fill price (`_last_fill_px` / `_last_fill_t`,
     captured from both IOC trade responses and WS fill events) and
     exposes it via `_recent_private_price()`. `_market_mid_from_book`
     now prefers the private signal when the cached book is stale or
     when the private fill is newer than the book snapshot. Affects
     disagreement gates (snipe + maker anchor) and relaxed-regime
     detection (quote sizing + position cap).

Everything else carries forward from strategy2: K=0 sit-out,
confidence-gated maker reprice cadence (12/8/4 s by K), strike-decay
edge model, FIFO-priority modify guard, sigma-scaled position cap.

Run:
    python day1/strategy3.py
...
class Strategy:
    ...
    def _record_informed_fill(self, counterparty: Optional[str],
        ...
        if fill_side == "buy":
            self._informed_strikes_bid = min(
                self.informed_strike_max, self._informed_strikes_bid + 1.0)
        else:
            self._informed_strikes_ask = min(
                self.informed_strike_max, self._informed_strikes_ask + 1.0)
    ...
    def _recent_private_price(self) -> Optional[float]:
        """v3: most-recent observed fill price from the PRIVATE feed (our
        IOC responses + WS fill events), if within the freshness window.

        The public feed (book snapshots, quote_add/cancel broadcasts)
        runs behind the private feed -- a fill we just observed on the
        private channel won't show up in the public book mid for tens of
        ms to ~200 ms. When fresh, this is the timeliest market signal
        available.

        Caveat: a single fill price is biased toward whichever side the
        trade printed against (bid hit vs ask lift). It's a noisier
        signal than a true mid, but the freshness wins in the regimes
        where we use it (disagreement gating + relaxed-regime detection).
        """
    ...
    def _market_mid_from_book(self) -> Optional[float]:
        """Market signal for anchor + disagreement gates. Prefers the
        freshest source between the public book mid and the most-recent
        private fill price (the public feed lags private by ~50-200 ms).

        Returns None silently when no sane signal is available -- callers
        treat that as 'no disagreement signal, proceed normally'."""
        private = self._recent_private_price()
        book = self._book_cache
        book_fresh = (book is not None and
                      time.time() - self._book_cache_t
                      <= self._book_cache_max_age_sec)
        if not book_fresh:
            # Public book stale: fall back to private signal if fresh.
            return private
        bids = book.get("bids") or []
        asks = book.get("asks") or []
        if not bids or not asks:
            # One-sided book: prefer the private fill if available.
            return private
        public_mid = (bids[0]["price"] + asks[0]["price"]) / 2.0
        # Both fresh: pick the more-recent one. Private fills arrive
        # ahead of the public book tick, so when they exist and are
        # newer than the cached book they're the better signal.
        if private is not None and self._last_fill_t > self._book_cache_t:
            return private
        return public_mid
    ...
    def desired_quotes(self) -> tuple[Optional[int], Optional[int], float, float]:
        ...
        bid_px = int(math.floor(fair - edge - extra_bid + skew))
        ask_px = int(math.ceil(fair + edge + extra_ask + skew))
