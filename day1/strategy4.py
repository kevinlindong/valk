"""
Day 1 live trading strategy v4.

Same Bayesian-posterior core as strategy3.py. v4 makes one structural
change driven by log analysis of the strategy3 trials, then adds the
new informed-trader feature requested by the user:

  REGRESSION FIX (vs strategy3): the v3 private-feed branch in
  `_market_mid_from_book` returned `_last_fill_px` as "market mid"
  whenever the most-recent fill was newer than the book cache. But our
  own fill price sits at our maker quote (one side of the spread), not
  at the mid -- so the value was biased by ~half a spread (5-15 ticks
  at typical sigma). Combined with v3's tightened
  `snipe_max_disagreement = 5.0`, the bias tripped the snipe
  disagreement gate near-permanently: across two strategy3 trials we
  fired ZERO IOC snipes (vs 23 in the prior v3 run, 14 in v2 baseline),
  losing the entire taker-side P&L contribution (~$500-1000/round in
  the rounds that worked). v4 restores book-only mid in
  `_market_mid_from_book`. `_last_fill_px`/`_last_fill_t` are kept as
  observability state (logged on status) but do not participate in the
  disagreement gates.

  NEW: INFORMED-TRADER OVERRIDE. If we know one trader has perfect
  information about the underlying (the values of `a` and `w` in
  X_i ~ Uniform(a, a+w), so they know the settlement), every fill we
  take against them is a signal about the true direction:
    - They bought from our ask at price P -> truth > P (we sold too low).
    - They sold into our bid at price P -> truth < P (we bought too high).
  v4 reacts:
    1. Cancel our resting order on the side that got picked off
       (don't sit and bleed more).
    2. Apply a cool-off (`informed_cooloff_sec`, default 15s) before we
       re-post on that side, so the strike sits during the danger
       window.
    3. Clamp our usable fair toward the truth-bound implied by their
       fill (max fair when they bought; min fair when they sold) for
       downstream desired_quotes() and snipe edge math.

  The informed trader id is configurable two ways:
    - Module constant `INFORMED_TRADER_ID` (edit this string to swap
      who the informed bot is).
    - Constructor param `informed_trader_id` (overrides the module
      constant; used by run_combined4's --informed-trader CLI flag).
  Setting `INFORMED_TRADER_ID = ""` (or None) disables the feature
  entirely; the strategy then behaves like strategy3 with the
  regression fix applied.

Everything else carries forward from strategy3: K=0 sit-out,
confidence-gated maker reprice cadence (12/8/4s by K), per-side
...
Run:
    python day1/run_combined4.py --informed-trader=ORACLE
While running: type 's'+Enter for status, 'f'+Enter to flatten,
'i'+Enter for informed-trader state, 'q'+Enter to quit. Ctrl-C also
flattens and exits cleanly.
"""
