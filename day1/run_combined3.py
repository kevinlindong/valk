"""
Run Strategy v3 (live trading) and Probe (passive market-data logger) together
over a SINGLE WebSocket connection.

The exchange allows only one private WS per API key, so you can't just run
strategy3.py and probe.py side-by-side -- one would steal the other's slot.
This runner shares one client and dispatches each WS event to both.

  - Strategy v3: keeps all v2 behaviors (K=0 sit-out, confidence-gated maker
                 reprice cadence, counterparty strike model) and hardens the
                 snipe path after second_round_loss.jsonl analysis:
                   * snipe_disagreement_directional = False (symmetric gate)
                   * snipe_max_disagreement = 5.0 (was 10.0)
                   * snipe_position_buffer = 5 lots below ±position_limit
                   * ERIK added to informed_counterparties
                 See strategy3.py docstring for the full rationale.
...
Run:
    python day1/run_combined3.py
    python day1/run_combined3.py --log day1/sess.jsonl
    python day1/run_combined3.py --symbol A
...
from strategy3 import URL, API_KEY, Posterior, Strategy, N_PRIOR_SIM  # noqa: E402
from probe import Probe  # noqa: E402


def main() -> None:
    parser = argparse.ArgumentParser(
        description="Run Strategy v3 + Probe together over one WS connection"
    )
    parser.add_argument("--symbol", default="A")
    parser.add_argument("--log", default=None,
                        help="JSONL log path. Default: "
                             "day1/logs/combined_log_v3_<YYYYMMDD_HHMMSS>.jsonl. "
    ...
    if args.log is None:
        ...
        args.log = f"day1/logs/combined_log_v3_{ts}.jsonl"
    ...
    try:
        while True:
            ...
            if cmd == "s":
                ...
                # v3: also surface strike counters and current cadence.
                ...
                print(f"  phase={strat.phase}  pos={strat.position}  "
                      ...
                      f"{marker}\n"
                      f"  v3: maker_reprice_every={interval_str}  "
