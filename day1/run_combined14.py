"""Run Strategy v14 + Probe together over one WS connection.

v14 vs v13: class-based CP weights from /api/admin/bots leak, plus
DIRECTIONAL position-targeting: when informed bias is meaningful, we
suspend the opposite-side maker quote and IOC through the book to
build position in the informed direction. Other interns are tracked
too. See strategy14.py for the full design.

Run:
    python day1/run_combined14.py
    python day1/run_combined14.py --log day1/sess.jsonl
    python day1/run_combined14.py --symbol A

Interactive commands:
    s    status (position, bias, target, fair)
...
from strategy14 import (  # noqa: E402
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Run Strategy v14 + Probe together over one WS connection")
    parser.add_argument("--symbol", default="A")
    parser.add_argument("--log", default=None,
                        help="JSONL log path. Default: "
                             "day1/logs/combined_log_v14_<YYYYMMDD_HHMMSS>.jsonl")
    args = parser.parse_args()

    if args.log is None:
        ts = time.strftime("%Y%m%d_%H%M%S", time.localtime())
        args.log = f"day1/logs/combined_log_v14_{ts}.jsonl"
    ...
    print(f"\nCombined v14 runner started.")
    print(f"  Strategy v14: live trading (symbol={args.symbol})")
        ...
        while True:
            ...
            if cmd == "s":
                ...
                print(f"  phase={strat.phase}  pos={strat.position}  "
                      ...
                      f"k={k}/{strat.n_total}\n"
                      f"  v14: precompute={n_scen}@cycle{pcycle} "
