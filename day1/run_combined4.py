"""
Run Strategy v4 (live trading) and Probe (passive market-data logger) together
over a SINGLE WebSocket connection.

The exchange allows only one private WS per API key, so you can't just run
strategy4.py and probe.py side-by-side -- one would steal the other's slot.
This runner shares one client and dispatches each WS event to both.

  - Strategy v4: keeps the v3 hardening (symmetric snipe gate at 5.0,
                 ±position buffer, K=0 sit-out, no-counterparty maker
                 gate). Reverts the v3 private-fill "market mid" branch
                 that shut off ALL snipes across two trial runs. Adds
                 an OPTIONAL informed-trader override: when a specified
                 trader is known to have perfect (a, w) knowledge, each
                 fill against them establishes a truth bound and (a)
                 cancels the picked-off side with a cool-off, (b) clamps
                 our usable fair so downstream maker quotes + snipes
                 respect the truth bound for the rest of the round.
                 See strategy4.py docstring for the full rationale.
...
Run:
    python day1/run_combined4.py
    python day1/run_combined4.py --log day1/sess.jsonl
    python day1/run_combined4.py --symbol A
    python day1/run_combined4.py --informed-trader ORACLE_NAME

The --informed-trader flag overrides strategy4.INFORMED_TRADER_ID. Pass
...
from strategy4 import URL, API_KEY, Posterior, Strategy, N_PRIOR_SIM  # noqa: E402
from probe import Probe  # noqa: E402


def main() -> None:
    parser = argparse.ArgumentParser(
        description="Run Strategy v4 + Probe together over one WS connection"
    )
    parser.add_argument("--symbol", default="A")
    parser.add_argument("--log", default=None,
                        help="JSONL log path. Default: "
                             "day1/logs/combined_log_v4_<YYYYMMDD_HHMMSS>.jsonl. "
    ...
    parser.add_argument("--informed-trader", default=None,
                        ...
                        help="Counterparty id of the informed trader who "
                             ...
                             "strategy4.INFORMED_TRADER_ID. Pass an empty "
                             "string to disable.")
    args = parser.parse_args()

    if args.log is None:
        ts = time.strftime("%Y%m%d_%H%M%S", time.localtime())
        args.log = f"day1/logs/combined_log_v4_{ts}.jsonl"
    ...
    print("Commands: 's'=status  'c'=counterparty stats  'i'=informed-trader  "
          "'f'=flatten  'q'=quit. Ctrl-C also flattens.\n")
    ...
    try:
        while True:
            ...
            if cmd == "s":
                ...
                print(f"  unknown command {cmd!r}; try s / c / i / f / q")
